...
#  Code from https://github.com/Pascalson/KGRL/blob/main/minigrid-kgrl/models/external_knowledge.py
...
def get_expert_actions(obs, expert_rules, env_name=""):
    ...
    expert_actions = torch.zeros((obs.image.shape[0], len(expert_rules), len(actions)), device=device)
    img = obs.image[:,:,:,0]
    colors = obs.image[:,:,:,1]
    states = obs.image[:,:,:,2]
